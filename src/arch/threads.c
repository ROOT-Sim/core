 ...
 * mm_freely, subject to the following restrictions:
...
#endif
{
	...
	/* The thread is responsible for mm_freeing the startup information */
	mm_free((void *)ti);
...
int thrd_create(thrd_t *thr, thrd_start_t func, void *arg)
{
	/* Fill out the thread startup information (passed to the thread wrapper,
	   which will eventually mm_free it) */
	_thread_start_info *ti = (_thread_start_info *) mm_alloc(sizeof(_thread_start_info));
	if(ti == NULL) {
		return thrd_nomem;
	}
	ti->mFunction = func;
...
int tss_set(tss_t key, void *val)
{
	...
	struct TinyCThreadTSSData *data = (struct TinyCThreadTSSData *) TlsGetValue(key);
	if(data == NULL) {
		data = (struct TinyCThreadTSSData *) mm_alloc(sizeof(struct TinyCThreadTSSData));
		if(data == NULL) {
			return thrd_error;
		}
