...
void distributed_mem_global_init(void)
{
	...
	size_t mem_size = distr_mem_total_size_compute();
	size_t chunks_count = mem_size / sizeof(struct distr_mem_chunk);
	...
	while(true) {
		...
		bool fail = false;
		if(ptr != ptr_max) {
			mem_deterministic_free((void *)ptr, mem_size);
			fail = mem_deterministic_alloc((void *)ptr_max, mem_size) != 0;
		}
		...
		if(!fail)
			mem_deterministic_free((void *)ptr_max, mem_size);

		ptr = ptr_max + mem_size;
	}
