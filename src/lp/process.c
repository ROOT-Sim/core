...
static inline void handle_remote_anti_msg(struct lp_ctx *lp, struct lp_msg *a_msg)
{
	...
	for(struct lp_msg *msg = pes_entry_msg_received(array_get_at(lp->p.pes, --i)); likely(msg->dest_t >= m_time);) {
		if(msg->raw_flags == m_id) {
			// Look for the correct rollback index (must be after the last validly processed message)
			while(i) {
			...
			// Avoid rescheduling the msg to cancel
