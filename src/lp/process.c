...
void process_lp_fini(void)
{
	...
	for(array_count_t i = 0; i < array_count(proc_p->p_msgs); ++i) {
		...
		msg = unmark_msg(msg);
		uint32_t flags =
		    atomic_load_explicit(&msg->flags, memory_order_relaxed);
		if(!(flags & MSG_FLAG_ANTI)) {
			if(flags & MSG_FLAG_PUBSUB) {
				stats_take(STATS_MSG_PUBSUB_COMMITTED, 1);
				pubsub_msg_free(msg);
			} else {
    ...
    array_count_t last_i, array_count_t past_i)
{
	...
	do {
		...
		ProcessEvent_pr(msg->dest, msg->dest_t, msg->m_type, msg->pl,
		    msg->pl_size, state_p);
	...
	for(array_count_t i = past_i; i < p_cnt; ++i) {
		...
		while(is_msg_sent(msg)) {
			...
			if(is_pubsub_msg(unmark_msg(msg))) {
				...
				pub_node_handle_published_antimessage(
				    unmark_msg(msg));
			} else
#endif
    ...
    const struct lp_msg *msg)
{
	...
	while(likely(past_i < p_cnt)) {
		...
		if(amsg->raw_flags >> MSG_FLAGS_BITS == m_id &&
		    amsg->m_seq == m_seq) {
...
{
	uint32_t m_id;
	if(likely(!array_count(early_antis) ||
		  !(m_id = msg->raw_flags >> MSG_FLAGS_BITS)))
		return false;
...
void process_msg(struct lp_msg *msg)
{
	...
	ProcessEvent_pr(msg->dest, msg->dest_t, msg->m_type, msg->pl,
	    msg->pl_size, this_lp->lib_ctx_p->state_s);
	stats_take(STATS_MSG_PROCESSED, 1);
	array_push(proc_p->p_msgs, msg);
