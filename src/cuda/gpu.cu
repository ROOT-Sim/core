...
int main()
{
	...
	for(int n = 1048576; n >= 16384; n /= 2) {
		...
		for(int r = 0; r < n_tests; r++) {
			...
			while(1) {
				...
				if(warm_up == 0 && time > 200) {
					...
					if(setup < 4) {
						nm_start(setup, initial_p1[setup], initial_p2[setup], initial_p3[setup],
						    current_rate);
...
#if(OPTM_SYNC == 1)
					kernel_handle_next_event<<<n_blocks, threads_per_block>>>(gvt, window_size,
					    d_inac_1, d_inac_2, d_inac_3, d_inac_4, d_inac_5, d_inac_6);
#else
					kernel_handle_next_event<<<n_blocks, threads_per_block>>>(gvt, h_lookahead,
					    d_inac_1, d_inac_2, d_inac_3, d_inac_4, d_inac_5, d_inac_6);
#endif
				...
				while(1) {
					...
					cudaMemcpy(&h_rollback_performed, d_rollback_performed, sizeof(char),
		...
		printf("%8d %5.2f %5.2f %5.2f %5.2f %5.2f %5.2f\n", n, mean_min, mean_avg, mean_max, i_min, i_avg,
		    i_max);
