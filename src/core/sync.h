...
#define mrswlock_rlock(lck_p)                                                                                          \
	__extension__({                                                                                                \
		do {                                                                                                   \
			int i = atomic_fetch_add_explicit((lck_p), -1, memory_order_relaxed);                          \
			if(likely(i > 0))                                                                              \
				break;                                                                                 \
			atomic_fetch_add_explicit((lck_p), 1, memory_order_relaxed);                                   \
			do {                                                                                           \
				spin_pause();                                                                          \
			} while(atomic_load_explicit((lck_p), memory_order_relaxed) <= 0);                             \
		} while(1);                                                                                            \
...
#define mrswlock_wlock(lck_p, r_cnt)                                                                                   \
	__extension__({                                                                                                \
		do {                                                                                                   \
			int i = atomic_fetch_add_explicit((lck_p), -(r_cnt), memory_order_relaxed);                    \
			...
			atomic_fetch_add_explicit((lck_p), (r_cnt), memory_order_relaxed);                             \
			do {                                                                                           \
				spin_pause();                                                                          \
			} while(atomic_load_explicit((lck_p), memory_order_relaxed) < 0);                              \
		} while(1);                                                                                            \
