...
bool sync_thread_barrier(void)
{
	...
	while (atomic_load_explicit(&c_leader, memory_order_acquire)) {}

	// Wait for all threads to synchronize
	atomic_fetch_sub_explicit(&c_1, 1U, memory_order_release);

	while (atomic_load_explicit(&c_1, memory_order_acquire)) {}

	// Leader election
	if (!atomic_fetch_add_explicit(&c_leader, 1U, memory_order_acq_rel)) {

		atomic_fetch_sub_explicit(&c_2, 1U, memory_order_release);

		// Wait all the other threads to leave the barrier
		while (atomic_load_explicit(&c_2, memory_order_acquire)) {}
