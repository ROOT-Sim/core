...
/// To be called from within ProcessPublishedEvent to complete the unpacking of
/// the message.
...
enum _topology_geometry_t {
	...
	TOPOLOGY_RING,        //!< a ring shaped topology walkable in a single
	                      //!< direction
	TOPOLOGY_BIDRING,     //!< a ring shaped topology
	TOPOLOGY_TORUS, //!< a torus shaped grid topology (a wrapping around
	                //!< square topology)
...
extern struct topology_t *TopologyInit(enum _topology_geometry_t geometry,
    unsigned int out_of_topology);
...
extern lp_id_t GetReceiver(const struct topology_t *topology, lp_id_t from,
