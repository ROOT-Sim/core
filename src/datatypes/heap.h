...
#define heap_insert_n(self, cmp_f, ins, n)                                                                             \
	__extension__({                                                                                                \
		...
		__typeof(array_items(self)) items = array_items(self);                                                 \
		for(__typeof(array_count(self)) j = n; j--;) {                                                         \
			__typeof(array_count(self)) i = array_count(self)++;                                           \
...
#define heap_extract_from(self, pos, cmp_f)                                                                            \
	__extension__({                                                                                                \
		...
		for(__typeof(array_count(self)) i = j * 2U + 1U; i < cnt; i = i * 2U + 1U) {                           \
 ...
 * For correct operation of the heap you need to always pass the same @a cmp_f both for insertion and extraction
 */
#define heap_extract(self, cmp_f) __extension__({ heap_extract_from(self, 0, cmp_f); })
...
#define heap_heapify_from_array(self, cmp_f)                                                                           \
	__extension__({                                                                                                \
		__typeof(array_items(self)) items = array_items(self);                                                 \
		__typeof(array_count(self)) cnt = array_count(self);                                                   \
		for(__typeof(array_count(self)) o = cnt / 2; o > 0; --o) {                                             \
			__typeof(array_count(self)) j = o - 1;                                                         \
			__typeof(*array_items(self)) elem = items[j];                                                  \
			for(__typeof(array_count(self)) i = j * 2U + 1U; i < cnt; i = i * 2U + 1U) {                   \
				i += i + 1U < cnt && cmp_f(items[i + 1U], items[i]);                                   \
