...
const double wall_step_s = 1.0;

const double min_param = 0.2;
const double max_param = 0.8;

const int forecast_steps = 10;
const double phi = 0.9;

int resample(struct data_point_raw *a, int len_a, struct data_point_raw *b, int len_b, int max_wall_step, double *dp_out) {
...
double compute_stddev(int len, double *samples)
{
  double sum = 0.0;
  for (int i = 0; i < len; i++)
    sum += samples[i];
...
double forecast(double alpha, double beta, int max_wall_step, double dp[max_wall_step], int train)
{
  alpha = clamp(alpha);
  beta = clamp(beta);
  ...
  for (int last_data_step = 0; last_data_step < max_wall_step - forecast_steps - 1; last_data_step++) {
    ...
    for (int h = 1; h < forecast_steps + 1; h++) {
      ...
      err += curr_h_err;
      //printf("error at %d + %d: %.2f - %.2f = %.2f\n", last_data_step, h, dp[last_data_step + h], forecast, fabs(dp[last_data_step + h] - forecast));
  ...
  for (int h = 1; h < forecast_steps + 1; h++) {
    ...
    //printf("forecast for step %d is %.2f\n", last_data_step, ss[last_data_step] + phi_sum * bs[last_data_step]);
  }

  double scale = wall_step_s / forecast_steps;
  printf("forecast over the next %.2f wall seconds is a difference of %.2f gvt units per wall second with stddev %.2f\n", wall_step_s * forecast_steps, sum_forecast * scale, err_stddev * scale);
...
double cmp_speeds(struct data_point_raw *a, int len_a, struct data_point_raw *b, int len_b) {
