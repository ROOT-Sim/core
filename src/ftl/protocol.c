...
static inline void reset_ftl_barrier(unsigned expected_threads, unsigned next_round_threads, unsigned expected_barrier,
    unsigned next_barrier_value)
{
	if(!__sync_bool_compare_and_swap(&ftl_curr_counter, expected_threads, next_round_threads)) {
		logger(LOG_WARN, "%u MY CAS FAILED AND THIS SHOULD NEVER HAPPEN expected %u vs %u", expected_threads,
		    ftl_curr_counter);
	}
	if(!__sync_bool_compare_and_swap(&ftl_spin_barrier, expected_barrier, next_barrier_value)) {
		logger(LOG_WARN, "%u MY CAS FAILED AND THIS SHOULD NEVER HAPPEN expected %u vs %u", expected_barrier,
		    ftl_spin_barrier);
	}
}
...
static inline int ftl_transition(enum ftl_phase curr, enum ftl_phase next)
{
	...
	if(!res) {
		logger(LOG_WARN, "%u MY CAS FAILED AND THIS SHOULD NEVER HAPPEN expected %u vs %u", curr,
		    current_phase);
	}
	return res;
}
...
void follow_the_leader(simtime_t current_gvt)
{
	...
	switch(current_phase) {
		case INIT:
			...
			if(rid == gpu_rid)
				logger(LOG_INFO, "GPU has initialized its components");
			...
			while(current_phase == INIT)
				; /// all threads should start a challenge by spinning, this should be fair
			return;
	...
	switch(current_phase) {
		case CPU:
			...
			val = __sync_add_and_fetch(&ftl_curr_counter, -1);
			while(val && ftl_spin_barrier == 1)
				; // all threads -1 will be stucked here
