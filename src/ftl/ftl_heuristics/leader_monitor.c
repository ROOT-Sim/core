...
bool leader_monitor(simtime_t current_gvt){
	...
	printf("CUR %f GVT %f - ema %f cur %f abs diff %.2f th %f \n", last_gvt, current_gvt, ema_speed, cur_speed, fabs(ema_speed - cur_speed)/ema_speed, LEADER_MONITOR_TH);
	...
	result = (ema_speed*LEADER_MONITOR_TH) < fabs(ema_speed - cur_speed);
	if(result) printf("Its time to start a challenge ema %f cur %f abs diff %.2f th %.2f \n", ema_speed, cur_speed, fabs(ema_speed - cur_speed)/ema_speed, LEADER_MONITOR_TH);
