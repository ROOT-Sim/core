...
static __thread simtime_t gvt_partial;
	...
	switch(thread_phase) {
		case thread_phase_A:
			if(atomic_load_explicit(&c_a, memory_order_relaxed))
				break;
			gvt_partial = min(gvt_partial, msg_queue_time_peek());
		...
		case thread_phase_C:
			if(atomic_load_explicit(&c_a, memory_order_relaxed) != global_config.n_threads)
				break;
			reducing_p[rid] = min(gvt_partial, msg_queue_time_peek());
